...
export default async function AssetAssignmentsPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Asset assignments" description="Assign an asset to one scoped student or employee at a time and record its return." />{hasPermission(user, "assets:update") ? <Card><CardHeader><CardTitle>Assign asset</CardTitle></CardHeader><CardContent><AssetAssignmentForm assets={assets} students={studentOptions} employees={employees} /></CardContent></Card> : null}<Card><CardHeader><CardTitle>Assignment history</CardTitle></CardHeader><CardContent>{assignments.length ? <div className="space-y-3">{assignments.map((assignment) => <div key={assignment.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><p className="font-medium">{assignment.assetName} <span className="text-xs text-muted-foreground">{assignment.assetCode ?? ""}</span></p><p className="text-xs text-muted-foreground">{assignment.assigneeType ?? "assignee"} · {assignment.assigneeId ?? "unknown"}</p></div><div className="flex items-center gap-3"><StatusBadge status={assignment.status} />{hasPermission(user, "assets:update") && assignment.status === "active" ? <AssetAssignmentStatusButton id={assignment.id} toStatus="returned" /> : null}</div></div>)}</div> : <p className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">No assignments found.</p>}</CardContent></Card></div>;
}
