...
export default async function ProcurementPurchaseOrdersPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Purchase orders" description="Create orders only from approved requisitions, then move them through controlled states." />{hasPermission(user, "procurement:create") ? <Card><CardHeader><CardTitle>New purchase order</CardTitle></CardHeader><CardContent><PurchaseOrderWithSupplierForm requisitions={approved} suppliers={suppliers} /></CardContent></Card> : null}<Card><CardHeader><CardTitle>Order history</CardTitle></CardHeader><CardContent>{orders.length ? <div className="space-y-3">{orders.map((order) => <div key={order.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><p className="font-medium">{order.name}</p><p className="text-xs text-muted-foreground">{order.code ?? order.id}</p></div><div className="flex items-center gap-3"><StatusBadge status={order.status} />{hasPermission(user, "procurement:approve") && nextOrderStatus[order.status] ? <ProcurementTransitionButton kind="order" id={order.id} toStatus={nextOrderStatus[order.status]} /> : null}</div></div>)}</div> : <p className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">No purchase orders found.</p>}</CardContent></Card></div>;
}
