...
export default async function UsersPage({
  ...
  return <div>
    ...
    <Card className="mt-6"><CardContent className="pt-6">
      <FilterBar summary={query.search ? `Searching for “${query.search}”` : "Search by name or email"}>
        <form action="/users" method="get" role="search" className="flex w-full max-w-md gap-2"><Input name="search" aria-label="Search users by name or email" defaultValue={query.search} placeholder="Search name or email" /><Button type="submit" variant="outline">Search</Button>{query.search ? <ButtonLink href="/users" variant="ghost" size="sm">Clear</ButtonLink> : null}</form>
      </FilterBar>
      ...
      ]} emptyTitle={query.search ? "No users match this search" : "No users found"} emptyDescription={query.search ? "Try a different name or email address." : "Invite a user to begin managing access."} filtered={Boolean(query.search)} ariaLabel="Users and access" caption="Users in the current authorized scope" />
