...
export default async function FeePaymentsPage() {
  ...
  const [payments, invoices, refundOptions] = await Promise.all([
    listPayments(user),
    getPaymentOptions(user),
    getRefundOptions(user),
  ]);
  return (
    <div>
      <PageHeader
        title="Fee payments"
        description="Post idempotent collections that atomically reduce invoice balance and create receipt and double-entry ledger records."
      />
      {hasPermission(user, "fees:collect") ? (
        <PaymentForm invoices={invoices} campusId={user.campusId} />
      ) : null}
      {hasPermission(user, "fees:refund") ? (
        <RefundForm payments={refundOptions} campusId={user.campusId} />
      ) : null}
      <Card>
        <CardContent className="pt-6">
          <DataTable
            ...
            emptyTitle="No fee payments found"
          />
        </CardContent>
      </Card>
    </div>
