...
export default async function HealthProfilesPage() {
  const user = await requirePermission("health:view_sensitive");
  const [students, profiles] = await Promise.all([
    listHealthStudents(user),
    listHealthProfiles(user),
  ]);
  return (
    <div className="space-y-6">
      <PageHeader
        title="Health profiles"
        description="Sensitive allergies and conditions are scoped to the health permission and current campus."
      />
      {hasPermission(user, "health:update") ? (
        <Card>
          <CardHeader>
            <CardTitle>Save profile</CardTitle>
          </CardHeader>
          <CardContent>
            <HealthProfileForm students={students} />
          </CardContent>
        </Card>
      ) : null}
      <Card>
