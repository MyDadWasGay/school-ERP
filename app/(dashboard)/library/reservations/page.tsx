...
export default async function LibraryReservationsPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Library reservations" description="Keep one pending reservation per user and catalogue item within the tenant scope." />{hasPermission(user, "library:update") ? <Card><CardHeader><CardTitle>Reserve an item</CardTitle></CardHeader><CardContent><LibraryReservationForm items={items} /></CardContent></Card> : null}<Card><CardHeader><CardTitle>Reservation queue</CardTitle></CardHeader><CardContent>{reservations.length ? <div className="space-y-2">{reservations.map((reservation) => <div key={reservation.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><div><p className="font-medium">{reservation.name}</p><p className="text-xs text-muted-foreground">Requested {formatIndiaDateTime(reservation.createdAt)}</p></div><StatusBadge status={reservation.status} /></div>)}</div> : <EmptyState title="No reservations found" description="Pending reservations will appear here when a user requests an unavailable catalogue item." />}</CardContent></Card></div>;
}
