...
export async function listInvoicesPage(user: CurrentUser, input?: { page?: number; pageSize?: number }) {
  const pagination = normalizePagination(input);
  const permittedIds = await resolvePermittedStudentIds(user);
  if (permittedIds && permittedIds.length === 0) {
    return { rows: [], pageInfo: { page: pagination.page, pageSize: pagination.pageSize, total: 0, pageCount: 0 } };
  }
  const where = and(
  ...
  const [rows, totals] = await Promise.all([
    getDb().select({
      ...
      .innerJoin(students, and(eq(students.id, feeInvoices.studentId), eq(students.organizationId, user.organizationId)))
      .where(where)
...
export async function getInvoiceStudentOptions(user: CurrentUser) {
  ...
  )).orderBy(students.firstName).limit(200);
...
export async function createInvoice(user: CurrentUser, input: InvoiceInput) {
  ...
  if (!enrollment) throw new AppError("CONFLICT", "The student needs an active enrollment before invoicing.", 409);
...
export async function listPayments(user: CurrentUser) {
  const permittedIds = await resolvePermittedStudentIds(user);
  if (permittedIds && permittedIds.length === 0) return [];
  const rows = await getDb().select().from(feePayments).where(and(
  ...
  )).orderBy(desc(feePayments.paidAt)).limit(100);
  return rows.map((row) => ({
    id: row.id,
...
export async function getRefundOptions(user: CurrentUser) {
  ...
  const rows = await getDb().select({
    ...
    .innerJoin(students, and(eq(students.id, feePayments.studentId), eq(students.organizationId, user.organizationId)))
    .leftJoin(feeRefunds, and(
    ...
    .groupBy(feePayments.id, feePayments.receiptNumber, feePayments.amountMinor, students.firstName, students.lastName)
    .orderBy(desc(feePayments.paidAt))
    .limit(200);
  return rows.flatMap((row) => {
    const remainingMinor = row.amountMinor - Number(row.refundedMinor ?? 0);
    return remainingMinor > 0 ? [{ id: row.id, label: `${row.receiptNumber} - ${row.firstName} ${row.lastName}`, remainingMinor }] : [];
  });
}
