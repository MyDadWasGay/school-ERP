...
import type {
  HostelAllotmentInput,
  HostelBedInput,
  HostelRoomInput,
} from "../schemas/hostel.schema";
...
async function getRoom(user: CurrentUser, roomId: string) {
  const room = await getDb().query.hostelRooms.findFirst({
    where: and(
  ...
  if (!room)
    throw new AppError(
      "NOT_FOUND",
      "Hostel room not found in your scope.",
      404,
    );
  return room;
}
...
export async function listHostelRooms(user: CurrentUser) {
  const rows = await getDb()
    ...
    .groupBy(
      hostelRooms.id,
      hostelRooms.building,
      hostelRooms.floor,
      hostelRooms.roomNumber,
      hostelRooms.capacity,
    )
    .orderBy(asc(hostelRooms.building), asc(hostelRooms.roomNumber));
  return rows.map((row) => ({
    ...row,
    available: Math.max(0, row.capacity - row.occupancy),
  }));
}
...
export async function createHostelRoom(
  ...
  const existing = await getDb().query.hostelRooms.findFirst({
    where: and(
      eq(hostelRooms.organizationId, user.organizationId),
      user.campusId
        ? eq(hostelRooms.campusId, user.campusId)
        : isNull(hostelRooms.campusId),
  ...
  if (existing)
    throw new AppError(
      "CONFLICT",
      "That room already exists in this campus.",
      409,
    );
  const [row] = await getDb()
    ...
    .returning();
  if (!row)
    throw new AppError("DATABASE_ERROR", "Unable to create hostel room.", 500);
  return row;
}
...
export async function listHostelBeds(user: CurrentUser) {
  const rows = await getDb()
    ...
    .orderBy(
      asc(hostelRooms.building),
      asc(hostelRooms.roomNumber),
      asc(hostelBeds.code),
    );
  return rows;
}

export async function createHostelBed(
  user: CurrentUser,
  input: HostelBedInput,
) {
  const room = await getRoom(user, input.roomId);
  const existing = await getDb().query.hostelBeds.findFirst({
    where: and(
      eq(hostelBeds.organizationId, user.organizationId),
  ...
  if (existing)
    throw new AppError(
      "CONFLICT",
      "That bed code already exists in this room.",
      409,
    );
  const [row] = await getDb()
    .insert(hostelBeds)
    ...
    .returning();
  if (!row)
    throw new AppError("DATABASE_ERROR", "Unable to create hostel bed.", 500);
  return row;
}
...
export async function allocateHostelBed(
  user: CurrentUser,
  input: HostelAllotmentInput,
) {
  const room = await getRoom(user, input.roomId);
  const bed = await getDb().query.hostelBeds.findFirst({
    where: and(
  ...
  if (!bed)
    throw new AppError("NOT_FOUND", "Bed not found in the selected room.", 404);
  ...
  if (!student)
    throw new AppError(
      "NOT_FOUND",
      "Student not found in the room campus.",
      404,
    );
  const existing = await getDb().query.hostelAllotments.findFirst({
    where: and(
      eq(hostelAllotments.organizationId, user.organizationId),
  ...
  if (existing)
    throw new AppError(
      "CONFLICT",
      "The student already has an active hostel allotment.",
      409,
    );
  ...
  if ((activeCount[0]?.value ?? 0) >= room.capacity)
    throw new AppError(
      "CONFLICT",
  ...
  try {
    const [row] = await getDb()
      ...
      .returning();
    if (!row)
      throw new AppError("DATABASE_ERROR", "Unable to allot hostel bed.", 500);
    return row;
  } catch (error) {
    ...
    if (
      databaseErrorIncludes(
        error,
        "hostel allotment capacity",
        "hostel_active_student_unique",
        "hostel_active_bed_unique",
      )
    ) {
      throw new AppError(
        "CONFLICT",
        "The student or bed is already allotted, or the room is at capacity.",
        409,
      );
    }
    throw error;
...
export async function listHostelAllotments(user: CurrentUser) {
  return getDb()
    ...
    .innerJoin(
      students,
      and(
        eq(students.id, hostelAllotments.studentId),
        eq(students.organizationId, user.organizationId),
      ),
    )
    .innerJoin(
      hostelRooms,
      and(
        eq(hostelRooms.id, hostelAllotments.roomId),
        eq(hostelRooms.organizationId, user.organizationId),
      ),
    )
    .leftJoin(
      hostelBeds,
      and(
        eq(hostelBeds.id, hostelAllotments.bedId),
        eq(hostelBeds.organizationId, user.organizationId),
      ),
    )
    .where(
    ...
    .orderBy(desc(hostelAllotments.allottedOn))
    .limit(500);
}

export async function checkoutHostelAllotment(
  user: CurrentUser,
  allotmentId: string,
) {
  const [row] = await getDb()
    .update(hostelAllotments)
    .set({
      status: "completed",
    ...
    .returning();
  if (!row)
    throw new AppError("NOT_FOUND", "Active hostel allotment not found.", 404);
  return row;
}
