...
type SeatRow = { sectionId: string; className: string; sectionName: string; capacity: number; occupied: number; available: number };
    ...
    <CardContent className="space-y-5">
      ...
      {rows.length ? <div className="space-y-3" aria-label="Seat occupancy by section">
        {rows.slice(0, 20).map((row) => {
          ...
          const status = row.available === 0 ? "full" : percent >= 90 ? "near full" : "open";
