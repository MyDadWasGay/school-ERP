...
import {
  addLibraryCopyAction,
  createLibraryItemAction,
  issueLibraryCopyAction,
  createDigitalResourceAction,
  reserveLibraryItemAction,
  renewLibraryCopyAction,
  returnLibraryCopyAction,
...
export function LibraryItemForm() {
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-3" noValidate>
    <div className="space-y-2"><Label htmlFor="library-title">Title</Label><Input id="library-title" name="title" required /></div>
    <div className="space-y-2"><Label htmlFor="library-author">Author</Label><Input id="library-author" name="author" /></div>
    <div className="space-y-2"><Label htmlFor="library-isbn">ISBN</Label><Input id="library-isbn" name="isbn" /></div>
    <div className="sm:col-span-3 flex items-center justify-between gap-3"><ActionMessage message={message} /><Button>Create catalogue item</Button></div>
  </form>;
}
...
export function LibraryCopyForm({ items }: { items: LibraryItem[] }) {
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2" noValidate>
    <div className="space-y-2"><Label htmlFor="library-copy-item">Catalogue item</Label><select id="library-copy-item" name="itemId" className="h-10 w-full rounded-md border bg-background px-3 text-sm" required><option value="">Choose an item</option>{items.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></div>
    <div className="space-y-2"><Label htmlFor="library-accession">Accession number</Label><Input id="library-accession" name="accessionNumber" required /></div>
    <div className="sm:col-span-2 flex items-center justify-between gap-3"><ActionMessage message={message} /><Button disabled={!items.length}>Add physical copy</Button></div>
  </form>;
}
...
export function IssueLibraryForm({ copies, borrowers }: { copies: LibraryCopy[]; borrowers: Borrowers }) {
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" noValidate>
    <div className="space-y-2"><Label htmlFor="library-issue-copy">Available copy</Label><select id="library-issue-copy" name="copyId" className="h-10 w-full rounded-md border bg-background px-3 text-sm" required><option value="">Choose a copy</option>{copies.map((copy) => <option key={copy.id} value={copy.id}>{copy.title} · {copy.accessionNumber}</option>)}</select></div>
    <div className="space-y-2"><Label htmlFor="library-borrower-type">Borrower type</Label><select id="library-borrower-type" className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={borrowerType} onChange={(event) => setBorrowerType(event.target.value as "student" | "user")}><option value="student">Student</option><option value="user">Staff/user</option></select></div>
    <div className="space-y-2"><Label htmlFor="library-borrower">Borrower</Label><select id="library-borrower" name="borrowerId" className="h-10 w-full rounded-md border bg-background px-3 text-sm" required><option value="">Choose a borrower</option>{options.map((borrower) => <option key={borrower.id} value={borrower.id}>{borrower.name}</option>)}</select></div>
    <div className="space-y-2"><Label htmlFor="library-due-date">Due date</Label><Input id="library-due-date" name="dueAt" type="date" defaultValue={defaultDueDate} required /></div>
    <div className="sm:col-span-2 lg:col-span-4 flex items-center justify-between gap-3"><ActionMessage message={message} /><Button disabled={!copies.length || !options.length}>Issue copy</Button></div>
  </form>;
}
...
export function ActiveLibraryIssues({ issues, canUpdate }: { issues: ActiveIssue[]; canUpdate: boolean }) {
  ...
  async function closeIssue(issue: ActiveIssue, outcome: "returned" | "lost" | "damaged") {
