const baseUrl = (process.env.JOB_WORKER_URL ?? process.env.API_INTERNAL_BASE_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? "").replace(/\/$/, "");
...
if (!baseUrl) throw new Error("JOB_WORKER_URL, API_INTERNAL_BASE_URL or NEXT_PUBLIC_APP_URL is required.");
...
const apiBaseUrl = (process.env.API_INTERNAL_BASE_URL ?? baseUrl).replace(/\/$/, "");
console.log(`Job worker polling ${apiBaseUrl}/api/v1/internal/jobs/run every ${intervalMs}ms.`);
while (!stopping) {
  await pollOnce();
...
async function pollOnce() {
  try {
    const response = await fetch(`${apiBaseUrl}/api/v1/internal/jobs/run`, {
      method: "POST",
